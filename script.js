...
function main() {
    ...
    const TEX = {};
    const TEX_FILES = ["grass_top.png", "grass_side.png", "dirt.png", "stone.png", "cobblestone.png", "oak_log_top.png", "oak_log_side.png", "oak_planks.png", "sand.png", "glass.png", "brick.png", "water.png", "leaves.png", "bedrock.png"];
    let texturesLoaded = 0;
    TEX_FILES.forEach(name => {
        texLoader.load("textures/block/" + name, tex => {
    ...
    renderer.domElement.addEventListener("click", () => {
        if (!pointerLocked && !inventoryOpen && !keybindMenuOpen && !pauseOpen) renderer.domElement.requestPointerLock();
    });
    ...
    function render() {
        ...
        document.getElementById("debug-info").innerHTML = `XYZ: ${player.x.toFixed(1)} / ${player.y.toFixed(1)} / ${player.z.toFixed(1)}<br>` + `${player.flying ? "FLYING" : "WALKING"} | ${player.onGround ? "Ground" : "Air"}<br>` + `Block: ${hotbar[selectedSlot]?.id ? BLOCKS[hotbar[selectedSlot].id].name : "Empty"}`;
    }
    // ============================================================
    // BLOCK ICON HELPER
    // ============================================================
...
function closeCursorOnInventoryClose() {
  ...
  if (hbSlots) {
    ...
    function fmt(code) {
        return code.replace("Key", "").replace("Digit", "").replace("ShiftLeft", "L-Shift").replace("ShiftRight", "R-Shift").replace("ControlLeft", "L-Ctrl").replace("Space", "Space").replace("ArrowUp", "↑").replace("ArrowDown", "↓").replace("ArrowLeft", "←").replace("ArrowRight", "→");
    }
