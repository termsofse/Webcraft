...
function main() {
    ...
    document.addEventListener("keydown", e => {
        ...
        if (action === "pause") {
            if (inventoryOpen) {
                inventoryOpen = false; updateInventoryUI(); renderer.domElement.requestPointerLock();
        ...
        el.addEventListener("mouseenter", e => {
            ...
            if ((e.buttons & 1) && dragButton === 0 && cursorStack) {
                ...
                if (!draggedSlots.includes(el)) {
                    isDragging = true;
                    draggedSlots.push(el);
                    applyRightDragStep(el, slotGetter, slotSetter);
                }
            }
        });
    ...
    function handleLeftClick(e, slotGetter, slotSetter, isCreativeSource) {
        ...
        if (!cursorStack) {
            cursorStack = { ...slotStack }; slotSetter(null);
        } else if (!slotStack) {
            slotSetter({ ...cursorStack }); cursorStack = null;
        } else if (cursorStack.id === slotStack.id) {
    ...
    function applyRightDragStep(el, slotGetter, slotSetter) {
    ...
    function buildKeybindUI() {
        ...
        for (const [action, label] of Object.entries(keybindLabels)) {
            ...
            [row.app](https://row.app)endChild(span);
            ...
            [row.app](https://row.app)endChild(btn);
            [list.app](https://list.app)endChild(row);
        }
    }
